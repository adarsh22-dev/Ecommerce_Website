        ...
        if (result.error) {
          ...
          if (authUser) {
            const { data: profile } = await supabase
              ...
              .eq("id", authUser.id)
              .single();
            if (profile && profile.role === "vendor" && profile.status === "pending") {
              toast.error("Your vendor account is pending admin approval. Please wait.");
              setLoading(false);
              return;
            }
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-12">
      ...
      >
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-foreground-secondary mt-2">Customer & Vendor Portal</p>
        </div>
        ...
        <div className="card p-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <Button type="submit" className="w-full shimmer-btn" size="lg" loading={loading}>
              {mode === "signin" ? "Sign In" : `Create ${accountType === "vendor" ? "Vendor" : "Customer"} Account`}
            </Button>
          </form>
